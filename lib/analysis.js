...
function analyzeChangeDetails (details) {
  ...
  for (let target of Object.keys(detailsByTarget)) {
...
function analyzeChangeDetailCause (modified, detail) {
  ...
  if (detail.Target.Attribute === 'Tags') {
    // Special case: tags are always changed as a result of direct modification
    // TODO: Verify it also holds for resources that have Tags defined as properties
    return [
      'changed stack tags'
    ]
  }
  ...
  switch (detail.ChangeSource) {
    ...
    default:
      throw new Error(`Unknwon ChangeSource ${detail.ChangeSource}`)
  }
}
